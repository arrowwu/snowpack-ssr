...
export default snowPackDevServer => async (filePath, options, callback) => {
  ...
  if (snowPackDevServer) { // import assets from dev server runtime
    ...
    htmlFile = readFileSync(path.join(path.resolve(), `/client/index.html`), 'utf8');
  } else { // import assets from build folder
  ...
  const pageProps = getPropsForServer ? await getPropsForServer({ api, options }) : null;
